...
class CourtDashboard2D:
    """2D球场面板"""

    def __init__(self,):
        self.axs = self._draw_ball_field(color='#20458C', lw=2)
        self.setup_court()

    def setup_court(self):
        """绘制半场篮球场2D平面图"""
        # 设置坐标轴范围
        self.axs.set_xlim(-250, 250)
        self.axs.set_ylim(422.5, -47.5)
        # 消除坐标轴刻度
        # self.axs.set_xticks([])
        # self.axs.set_yticks([])
        # 添加备注信息
        plt.annotate('By Greyson', xy=(100, 160), xytext=(178, 418))

    @staticmethod
    def _draw_ball_field(color='#20458C', lw=2):
        """
        绘制篮球场
        """
        # 新建一个大小为(6,6)的绘图窗口
        ...
        # 对篮球场进行底色填充
        lines_outer_rec = Rectangle(xy=(-250, -47.5), width=500, height=470, linewidth=lw, color='#F0F0F0',
                                    fill=True)
        # 设置篮球场填充图层为最底层
        ...
        three_arc = Arc(xy=(0, 0), width=477.32, height=477.32, theta1=22.8, theta2=157.2, linewidth=lw,
                        color=color, fill=False)
        # 将arc添加进ax
        ax.add_patch(three_arc)

        # 绘制中场处的外半圆,半径为60
        center_outer_arc = Arc(xy=(0, 422.5), width=120, height=120, theta1=180, theta2=0, linewidth=lw,
                               color=color, fill=False)
        # 将arc添加进ax
    ...
    def shot_attempt(self, x, y, make=False):
        """在球场图上绘制投篮尝试"""

        # 绘制散点图
        if make:
            self.axs.scatter(x=x, y=y, s=30, marker='o', edgecolors='#3A7711', color="#F0F0F0",
                             linewidths=2)
        else:
            self.axs.scatter(x=x, y=y, s=30, marker='x', color='#A82B2B')
    ...
    def save_dashboard(self, filename='shot_analysis_2d.png'):
        """保存面板为图片"""
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        print(f"2D分析图已保存: {filename}")
...
if __name__ == '__main__':
    dashboard_2d = CourtDashboard2D()
    dashboard_2d.shot_attempt(30,30, False)
    dashboard_2d.show()